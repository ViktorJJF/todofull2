const model = require('../models/EcommercesOrders');
...
const list = async (req, res) => {
  try {
    let relationalQuery = await db.checkQueryStringRelational(
      req,
      EcommercesContacts,
      'ecommercesContactId',
    );
    res.status(200).json(await db.getItems(req, model, relationalQuery));
  } catch (error) {
    utils.handleError(res, error);
  }
};
