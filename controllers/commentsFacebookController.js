const model = require('../models/CommentsFacebook');
...
const create = async (req, res) => {
  try {
    ...
    let isTemplate = req.body.type == 'template';
    const doesItemExists = isTemplate ? false : await itemExists(req.body);
    if (!doesItemExists || isTemplate) {
...
const update = async (req, res) => {
  try {
    ...
    let isTemplate = req.body.type == 'template';
    const id = await utils.isIDGood(req.params.id);
    const doesItemExists = isTemplate
      ? false
      : await itemExistsExcludingItself(id, req.body);
    if (!doesItemExists || isTemplate) {
